...
impl<'a> ECDSAKeyPair {
    ...
    /// sign message
    pub fn sign(&self, _rng: &rand::SecureRandom, msg: untrusted::Input, _signature: &mut [u8])
        ...
        //    or obtained from a trusted third party,
        // 3. assurance of the validity of the public key Q, and
        // 4. assurance that he/she/it actually possesses the associated private
        //    key d

        loop {
            loop {
                // NSA Guide Step 1: "Use one of the routines in Appendix A.2 to generate
                // (k, k**−1), a per-message secret number and its inverse modulo n. Since
                // n is prime, the output will be invalid only if there is a failure in the RBG"


                // NSA Guide Step 2: "Compute the elliptic curve point R = kG = (xR, yR)
                // using EC scalar multiplication (see [Routines]), where G is the base point
                // included in the set of domain parameters."


                // NSA Guide Step 3: "Compute r = xR mod n. If r = 0, then return to Step 1"

                break
            }

            // NSA Guide Step 4: "Use the selected hash function to compute H =
            // Hash(M)."
            // NSA Guide Step 5: "Convert the bit string H to an integer e as
            // described in Appendix B.2."
            let e = digest_scalar(self.alg.scalar_ops, self.alg.digest_alg, msg);

            // NSA Guide Step 6: "Compute s = (k − 1 ∗ (e + d ∗ r)) mod n. If s = 0, return to Step 1."

            // NSA Guide Step 7: "Return (r, s)"
